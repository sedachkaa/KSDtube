...
--filter-tcp=443 --hostlist="%LISTS%list-google.txt" --ip-id=zero --dpi-desync=multidisorder --dpi-desync-split-pos=1,midsld --dpi-desync-repeats=16 --dpi-desync-ttl=4 --dpi-desync-fooling=md5sig --dpi-desync-fake-tls-mod=padencap --dpi-desync-fake-tls="%BIN%tls_clienthello_www_google_com.bin" --new ^
...
--filter-udp=443 --hostlist="%LISTS%list-google.txt" --ipset="%LISTS%ipset-all.txt" --hostlist-exclude="%LISTS%list-exclude.txt" --hostlist-exclude="%LISTS%list-exclude-user.txt" --ipset-exclude="%LISTS%ipset-exclude.txt" --ipset-exclude="%LISTS%ipset-exclude-user.txt" --dpi-desync=multidisorder --dpi-desync-repeats=16 --dpi-desync-fake-quic="%BIN%quic_initial_www_google_com.bin" --new ^
